...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["update_status"])) {
        ...
        db_query("UPDATE inquiries SET status = ? WHERE id = ?", [
            $new_status,
            $inq_id,
        ]);
        $msg = "Status updated.";
    ...
    if (isset($_POST["delete"])) {
        db_query("DELETE FROM inquiries WHERE id = ?", [
            intval($_POST["delete"]),
        ]);
        $msg = "Lead deleted.";
    }
    ...
    if (isset($_POST["export"])) {
        ...
        $leads = db_get_all(
            "SELECT i.*, c.title as course_title FROM inquiries i LEFT JOIN courses c ON i.course_id = c.id $where ORDER BY i.created_at DESC",
            $params
        );
...
<div>
    <div class="flex justify-between items-center mb-5">
        <div>
            ...
            <p class="text-sm text-slate-500">Total: <?= count(
                $inquiries
            ) ?> leads</p>
        </div>
    ...
    <form method="GET" class="mb-5 flex flex-wrap gap-3 items-end bg-white p-4 rounded-2xl border">
        <div>
            ...
            <select name="status" class="border px-3 py-2 rounded-lg text-sm">
                ...
                <option value="Pending" <?= $filter_status == "Pending"
                    ? "selected"
                    : "" ?>>Pending</option>
                <option value="Contacted" <?= $filter_status == "Contacted"
                    ? "selected"
                    : "" ?>>Contacted</option>
                <option value="Enrolled" <?= $filter_status == "Enrolled"
                    ? "selected"
                    : "" ?>>Enrolled</option>
                <option value="Closed" <?= $filter_status == "Closed"
                    ? "selected"
                    : "" ?>>Closed</option>
            </select>
        </div>
        <div class="flex-1 min-w-[200px]">
            ...
            <input type="text" name="search" value="<?= htmlspecialchars(
                $search
            ) ?>" class="border px-3 py-2 w-full rounded-lg text-sm" placeholder="Search leads...">
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if ($inq["email"]): ?>
                            <div class="text-xs text-slate-500"><?= htmlspecialchars(
                                $inq["email"]
                            ) ?></div>
                        <?php endif; ?>
                    </td>
                    <td class="text-xs"><?= htmlspecialchars(
                        $inq["course_title"] ?? "General Inquiry"
                    ) ?></td>
                    <td class="text-xs max-w-[220px]">
                        <div class="line-clamp-2"><?= htmlspecialchars(
                            substr($inq["message"], 0, 120)
                        ) ?>...</div>
                    </td>
                    <td>
                        <form method="POST" class="flex gap-1 items-center">
                            <input type="hidden" name="inq_id" value="<?= $inq[
                                "id"
                            ] ?>">
                            <select name="new_status" class="text-xs border px-2 py-1 rounded">
                                <option value="Pending" <?= $inq["status"] ==
                                "Pending"
                                    ? "selected"
                                    : "" ?>>Pending</option>
                                <option value="Contacted" <?= $inq["status"] ==
                                "Contacted"
                                    ? "selected"
                                    : "" ?>>Contacted</option>
                                <option value="Enrolled" <?= $inq["status"] ==
                                "Enrolled"
                                    ? "selected"
                                    : "" ?>>Enrolled</option>
                                <option value="Closed" <?= $inq["status"] ==
                                "Closed"
                                    ? "selected"
                                    : "" ?>>Closed</option>
                            </select>
                    ...
                    <td class="text-xs text-slate-500"><?= date(
                        "d M Y",
                        strtotime($inq["created_at"])
                    ) ?></td>
                    <td class="text-right">
                        <a href="tel:<?= $inq[
                            "phone"
                        ] ?>" class="text-xs px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded">Call</a>
                        <form method="POST" class="inline ml-1" onsubmit="return confirm('Delete this lead?')">
                            <input type="hidden" name="delete" value="<?= $inq[
                                "id"
                            ] ?>">
