...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["delete"])) {
        db_query("DELETE FROM testimonials WHERE id=?", [
            intval($_POST["delete"]),
        ]);
        $msg = "Testimonial deleted.";
    } elseif (isset($_POST["save"])) {
        $name = content_limit(trim($_POST["name"] ?? ""), 100);
        $role = content_limit(trim($_POST["role"] ?? ""), 150);
        $quote = content_limit(trim($_POST["quote"] ?? ""), 600);
        ...
        if (!empty($_POST["id"])) {
            db_query(
                "UPDATE testimonials SET name=?, role=?, quote=?, rating=?, photo=?, status=? WHERE id=?",
                [
                    $name,
                    $role,
                    $quote,
...
$edit = isset($_GET["edit"])
    ? db_get_row("SELECT * FROM testimonials WHERE id=?", [
        intval($_GET["edit"]),
    ])
    : null;
?>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input name="role" maxlength="150" value="<?= e(
                        $edit["role"] ?? ""
                    ) ?>" class="w-full border px-3 py-2 rounded-xl" placeholder="Parent of Aarav">
                </div>
            </div>

            <div class="mt-4">
                <label class="block text-xs font-semibold mb-1">Testimonial Quote</label>
                <textarea name="quote" rows="3" maxlength="600" class="w-full border px-3 py-2 rounded-xl" required><?= e(
                    $edit["quote"] ?? ""
                ) ?></textarea>
                <div class="text-[10px] text-slate-500 mt-1">Maximum 600 characters.</div>
            </div>
    ...
    : "" ?>><?= $i ?> ★</option>
                ...
                <div>
                    ...
                    <select name="status" class="w-full border px-3 py-2 rounded-xl">
                        <option value="active" <?= ($edit["status"] ??
                            "active") ===
                        "active"
                            ? "selected"
                            : "" ?>>Active</option>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="text-xs text-slate-500"><?= htmlspecialchars(
                            $t["role"]
                        ) ?></td>
                        <td class="text-xs max-w-[280px] truncate"><?= htmlspecialchars(
                            substr($t["quote"], 0, 80)
                        ) ?>...</td>
                        ...
                        <td>
                            <div class="flex gap-1">
                                <a href="?edit=<?= $t[
                                    "id"
                                ] ?>" class="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded">Edit</a>
