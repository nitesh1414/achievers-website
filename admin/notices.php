...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["delete"])) {
        ...
        $content = content_limit(trim($_POST["content"] ?? ""), 1500);
        $publish_date = $_POST["publish_date"];
        $status = $_POST["status"];

        $file_path = str_replace("../", "", $_POST["old_file"] ?? "");
        ...
        if (!empty($_POST["id"])) {
            db_query(
                "UPDATE notices SET title=?, content=?, file_path=?, publish_date=?, status=? WHERE id=?",
                [
                    $title,
                    $content,
                    $file_path,
            ...
            db_query(
                ...
                [$title, $content, $file_path, $publish_date, $status]
            );
            $msg = "Notice added.";
        }
    }
...
$edit = isset($_GET["edit"])
    ? db_get_row("SELECT * FROM notices WHERE id=?", [intval($_GET["edit"])])
    : null;
?>
<div>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <?php if ($edit): ?>
                ...
                <input type="hidden" name="old_file" value="<?= "../" .
                    htmlspecialchars($edit["file_path"]) ?>">
            <?php endif; ?>
            
            <div><label class="block text-xs font-semibold">Title</label><input name="title" value="<?= htmlspecialchars(
                $edit["title"] ?? ""
            ) ?>" class="w-full border px-3 py-2 rounded-xl" required></div>
            
            <div class="grid grid-cols-2 gap-4 mt-3">
                <div><label class="block text-xs font-semibold">Publish Date</label><input type="date" name="publish_date" value="<?= $edit[
                    "publish_date"
                ] ??
                    date(
                        "Y-m-d"
            ...
            <div class="mt-3"><label class="block text-xs font-semibold">Content / Announcement</label><textarea name="content" class="w-full border px-3 py-2 rounded-xl" rows="3"><?= htmlspecialchars(
                $edit["content"] ?? ""
            ) ?></textarea></div>
            ...
            <div class="mt-3">
                ...
                <?php if ($edit && $edit["file_path"]): ?>
                    <div class="mb-1 text-xs"><a href="<?= "../" .
                        htmlspecialchars(
                            $edit["file_path"]
                        ) ?>" target="_blank" class="text-blue-500">View current file</a></div>
                <?php endif; ?>
        ...
        <table class="admin-table w-full text-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php if ($n["file_path"]): ?><a href="<?= "../" .
    $n[
        "file_path"
    ] ?>" target="_blank" class="text-blue-500">Download</a><?php else: ?>—<?php endif; ?></td>
