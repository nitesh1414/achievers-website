...
$address = get_setting('address', 'Plot No. 45, Wardhaman Nagar, Nagpur - 440008');
...
<div class="tail-container max-w-[1200px] mx-auto px-6 py-12">
    ...
    <div class="text-center mb-12">
        ...
        <h1 class="text-4xl md:text-5xl font-bold text-slate-900 tracking-tighter">Contact Achievers Academy</h1>
        <p class="mt-3 text-lg text-slate-600 max-w-md mx-auto">We're here to answer your questions and help you get started on your gymnastics journey.</p>
    </div>
    ...
    <div class="grid lg:grid-cols-12 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex gap-4">
                        ...
                        <div>
                            ...
                            <div class="text-slate-800 leading-snug"><?= htmlspecialchars($address) ?></div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <div>
                            ...
                            <a href="tel:<?= htmlspecialchars($phone) ?>" class="block text-lg font-medium hover:text-amber-600 transition-colors">
                    ...
                    <div class="flex gap-4">
                        ...
                        <div>
                            ...
                            <a href="https://wa.me/<?= htmlspecialchars($whatsapp) ?>?text=Hi%20Achievers%20Academy" target="_blank"
                    ...
                    <div class="flex gap-4">
                        ...
                        <div>
                            ...
                            <a href="mailto:<?= htmlspecialchars($email) ?>" class="block text-lg font-medium hover:text-blue-600 transition-colors">
                ...
                <div class="mt-8 pt-6 border-t">
                    ...
                    <div class="flex gap-3">
                        <?php if ($ig = get_setting('instagram')): ?>
                            <a href="<?= htmlspecialchars($ig) ?>" target="_blank"
                                class="w-10 h-10 flex items-center justify-center bg-slate-100 hover:bg-pink-100 text-pink-600 hover:text-pink-700 rounded-2xl transition-colors"
                        ...
                        <?php if ($fb = get_setting('facebook')): ?>
                            <a href="<?= htmlspecialchars($fb) ?>" target="_blank"
                                class="w-10 h-10 flex items-center justify-center bg-slate-100 hover:bg-blue-100 text-blue-600 hover:text-blue-700 rounded-2xl transition-colors"
                                aria-label="Facebook">
                        ...
                        <?php if ($li = get_setting('linkedin')): ?>
                            <a href="<?= htmlspecialchars($li) ?>" target="_blank"
                                class="w-10 h-10 flex items-center justify-center bg-slate-100 hover:bg-sky-100 text-sky-600 hover:text-sky-700 rounded-2xl transition-colors"
                                aria-label="LinkedIn">
                        ...
                        <a href="https://wa.me/<?= htmlspecialchars($whatsapp) ?>" target="_blank"
                            class="w-10 h-10 flex items-center justify-center bg-slate-100 hover:bg-green-100 text-green-600 hover:text-green-700 rounded-2xl transition-colors"
                            aria-label="WhatsApp">
        ...
        <div class="lg:col-span-7">
            <div class="bg-white border border-slate-200 shadow-sm rounded-3xl p-8">
                <h3 class="font-bold text-xl mb-1">Send us a quick message</h3>
                <p class="text-sm text-slate-500 mb-6">We'll get back to you within a few hours.</p>
            ...
            <div class="rounded-3xl overflow-hidden border border-slate-200 shadow-sm">
                <div class="aspect-video w-full">
                    ...
                    if (!empty($map_html)) {
                        ...
                        $map_html = html_entity_decode($map_html, ENT_QUOTES | ENT_HTML5);
                        $map_html = htmlspecialchars_decode($map_html, ENT_QUOTES | ENT_HTML5);

                        // Strip any remaining escaped tags
                        $map_html = str_replace(['&lt;iframe', '&lt;/iframe&gt;', '&amp;lt;', '&amp;gt;'], ['<iframe', '</iframe>', '<', '>'], $map_html);

                        // If user pasted only a URL instead of full iframe
                        if (stripos($map_html, '<iframe') === false && (filter_var($map_html, FILTER_VALIDATE_URL) || stripos($map_html, 'google') !== false)) {
                            $src = htmlspecialchars($map_html, ENT_QUOTES, 'UTF-8');
                            echo '<iframe src="' . $src . '" width="100%" height="100%" style="border:0;" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>';
                        } else {
